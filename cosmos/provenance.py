...
# -------------------------------------------------- provenance that travels (V2)

#: Where a reader of an exported file is sent for the whole story.
HOME = "Reference → Data & methods, in the Cosmos application."


def describe_sources(simulator_id: str) -> str:
    """One honest line about what a simulator can plot, when it does not say itself.

    A simulator that offers a real sample *and* a synthetic one cannot be summarised
    without knowing which is on screen, so this says both are possible rather than
    guessing. Simulators that can tell the difference override it.
    """
    real, made = sources_for(simulator_id)
    if real and made:
        return ("This simulator plots both real measurements ("
                + ", ".join(m.title for m in real) + ") and data generated by the app ("
                + ", ".join(g.title.lower() for g in made) + "). "
                "Which one this file holds depends on what was selected.")
    if real:
        return ("Real measurements: "
                + "; ".join(f"{m.title} — {m.cite}" for m in real) + ".")
    if made:
        return ("Generated by Cosmos, not measured: "
                + ", ".join(g.title.lower() for g in made) + ".")
    return "Computed by Cosmos from the model, not measured."
...
def export_note(
    ...
    *,
    ...
    """The lines that travel with an exported plot or table.

    An exported file outlives the window it came from. Without this it is a
    picture of some curves, and nobody can tell a measurement from something the
    app invented to illustrate one.
    """
    from cosmos import APP_NAME, __version__

    lines = [f"{APP_NAME} {__version__} — {plot_name}"]
    if simulator_id:
        title = f"Simulator {simulator_id}"
        lines.append(f"{title} · {simulator_title}" if simulator_title else title)
    lines.extend(detail or [describe_sources(simulator_id)])
    lines.append(f"Exported {exported or date.today().isoformat()}. {HOME}")
    return lines
