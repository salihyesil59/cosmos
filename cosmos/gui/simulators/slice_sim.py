...
class SliceSimulator(SimulatorBase):
    ...
    def provenance(self, plot: str = "") -> list[str]:
        """V2: four tabs of mock and one of real sky, which is the point of S23."""
        if plot == "sdss_slice":
            return ["Real measurements: SDSS DR18 spectroscopic galaxies "
                    "(Almeida et al. 2023, ApJS 267, 44), the equatorial strip "
                    "120° < RA < 250°, |dec| < 1.5°, 0.005 < z < 0.15.",
                    "Anything on this plot labelled as the mock is the lognormal catalogue "
                    "generated by Cosmos, put beside the sky on purpose."]
        s = self.settings()
        return [
            "Generated by Cosmos, not measured: a lognormal mock catalogue — a Gaussian "
            "field exponentiated into a positive density, Poisson-sampled for galaxies, "
            "then observed. Not an N-body simulation.",
            f"Mock: seed {s.seed}, bias {s.bias:.2f}, density {s.density:g}, "
            f"wedge {s.wedge_deg:.0f}° × {s.thickness_deg:.1f}° out to {s.r_max:.0f} Mpc/h, "
            f"peculiar velocities {'on' if s.velocities else 'off'}, "
            f"fingers of God {s.fingers_km_s:.0f} km/s, redshift error {s.redshift_error:.4f}.",
        ]
