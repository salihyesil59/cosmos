...
class MCMCSimulator(SimulatorBase):
    ...
    def provenance(self, plot: str = "") -> list[str]:
        """V2: a posterior is only as real as the likelihood underneath it."""
        s = self.sample
        if s.real:
            source = f"Real measurements: {s.label}. {s.citation}. Diagonal errors only."
        else:
            source = f"Generated by Cosmos, not measured: {s.label}, {len(s.z)} supernovae."
        chain = (f"Chain: {int(self.steps.value())} steps, proposal σ = {self.step_size.value():.3f}, "
                 f"seed {int(self.seed.value())}, "
                 f"{'flat' if self.flat.isChecked() else 'Ωm–ΩΛ'} parameter space, "
                 f"extra probe: {self.probe()}.")
        return [source, chain]
