...
class CurvatureSimulator(SimulatorBase):
    def __init__(self, info, parent=None):
        ...
        shapes = QGroupBox("1 · Shapes drawn in space")
        sl = QVBoxLayout(shapes)
        self.side = ParameterSlider(
            "Triangle side (curvature radii)", 0.05, 2.0, 1.0, decimals=2, step=0.05,
            info=("Triangle size", "Side length of an equilateral triangle, measured in units of the radius of "
                  "curvature R. Small triangles look flat in every geometry; large ones reveal the curvature."),
        )
        self.radius = ParameterSlider(
            "Circle radius (curvature radii)", 0.05, 3.0, 1.0, decimals=2, step=0.05,
            info=("Circle radius", "Distance from the centre to the circle, measured along the curved space."),
        )
        sl.addWidget(self.side)
        sl.addWidget(self.radius)
        self.controls.addWidget(shapes)

        universe = QGroupBox("2 · Our universe")
        ul = QVBoxLayout(universe)
        self.omega_k = ParameterSlider(
            "Ωk  curvature parameter", -0.1, 0.1, -0.01, decimals=3, step=0.001,
            info=("Curvature parameter", "Ωk < 0 closed (spherical), Ωk > 0 open (hyperbolic). Measurements give "
                  "|Ωk| < 0.002."),
        )
        self.distance = ParameterSlider(
            "Distance to a ruler (curvature radii)", 0.1, 3.0, 1.0, decimals=2, step=0.05,
            info=("Ruler distance", "How far away a ruler of fixed length is. Its apparent angle depends on the "
        ...
        tabs.addTab(self.triangles, "Triangles")
        tabs.addTab(self.circles, "Circles")
        tabs.addTab(self.sizes, "Apparent sizes")
