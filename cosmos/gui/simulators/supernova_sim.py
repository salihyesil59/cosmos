...
class SupernovaSimulator(SimulatorBase):
    ...
    def provenance(self, plot: str = "") -> list[str]:
        """V2: three samples, and only one of them was measured by anybody."""
        s = self.sample
        if s.real:
            source = (f"Real measurements: {s.label}. {s.citation}. Diagonal errors only, so the "
                      "fit here is a teaching fit rather than the published cosmology.")
        else:
            source = (f"Generated by Cosmos, not measured: {s.label}, {len(s.z)} supernovae drawn "
                      f"to resemble a real survey. {s.description}")
        om, ol = self.best()
        fit = (f"Best fit on this plot: Ωm = {om:.2f}, ΩΛ = {ol:.2f}"
               + (" (flatness assumed)" if self.flat.isChecked() else "")
               + f", H0 = {self.hubble_constant():.1f} km/s/Mpc "
                 f"with M = {self.absolute_magnitude():.3f}.")
        return [source, fit]
