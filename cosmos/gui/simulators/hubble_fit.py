...
class HubbleFitSimulator(SimulatorBase):
    ...
    def provenance(self, plot: str = "") -> list[str]:
        """V2: which of the two samples the exported file actually holds."""
        ds = self.data
        if ds.simulated:
            source = f"Generated by Cosmos, not measured: {ds.label}."
        else:
            source = "Real measurements: Hubble's 1929 galaxies (Hubble 1929, PNAS 15, 168)."
        line = f"Your line: H0 = {self.h0.value():.1f} km/s/Mpc"
        if self.fit is not None:
            line += f"; least-squares fit: H0 = {self.fit.H0:.1f} km/s/Mpc"
        return [source, line + "."]
